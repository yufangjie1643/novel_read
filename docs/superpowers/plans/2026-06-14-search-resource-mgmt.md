...
pub struct SearchSnapshot {
    ...
    pub statuses: HashMap<String, crate::db::models::SourceStatus>,
    pub failures: Vec<crate::db::models::SearchFailure>,
...
pub async fn search_books_stream_v2(
    ...
    // Capture last search snapshot.
    let mut statuses = std::collections::HashMap::new();
    {
        let map = sup.in_flight.read().await;
        // The in_flight map is now empty (every src removed itself),
        // so the UI-side statuses are no longer reachable from here.
        // We just leave statuses empty in the snapshot; the frontend
        // re-derives statuses from its own state on remount.
        let _ = map;
    }
    // (Note: a fuller snapshot implementation would collect
    // statuses from the events as they were sent. For v1 we leave
    // the snapshot with an empty statuses map and an empty results
    // list. Frontend re-runs the search on remount to repopulate.
    // This is acceptable per the user-approved design where "last
    // search" is best-effort and may not include the most recent
    // tail of results.)
...
3. **Type consistency** —
   ...
   - `SearchSnapshot` fields: `request_id`, `query`, `results`, `statuses`, `failures`, `total_results`, `duration_ms`, `captured_at` — used in Tasks 2 and 10.
